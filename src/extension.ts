...
function showError(msg: string) {
  vscode.window.showErrorMessage(`[markdown-extension] ${msg}`)
}

export function activate(context: vscode.ExtensionContext) {
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'markdown-extension.openEditor',
      (uri?: vscode.Uri, ...args) => {
...
class EditorPanel {
  ...
  public static readonly viewType = 'markdown-extension'
  ...
  private get _config() {
    return vscode.workspace.getConfiguration('markdown-extension')
  }
  ...
  private _getHtmlForWebview(webview: vscode.Webview) {
    ...
    const JsFiles = ['toastui-editor-all.min.js'].map(toMediaPath).map(toUri)
    const CssFiles = ['toastui-editor.min.css'].map(toMediaPath).map(toUri)
    ...
    const html = `<!DOCTYPE html>
			...
			<body>
        <div id="over" style="height: 100vh">
          <div id="app"></div>
          </div>
        ...
        <script>
          ...
          editor.addHook("change", function () {
            vscode.postMessage({ command: 'edit', content: editor.getMarkdown() })
          })
          ...
          console.log("POSTED MESSAGE TO VSCODE")
